...
subtitle: "Evidence from colleges in India"
...
plot <-
  ...
  select(ends_with("seg_new"), state = state_india, reservation) %>% 
  group_by(state) %>% 
  summarize(
    b_seg_new = mean(b_seg_new, na.rm = T),
    e_seg_new = mean(e_seg_new, na.rm = T)
  ) %>%
  full_join(
  ...
  ggplot(aes(text = state, fill = e_seg_new), size = 0.3) +
  geom_sf() +
